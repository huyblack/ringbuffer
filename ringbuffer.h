 ...
 */
 ...
 /**
 * @brief Write data to a ring buffer, overwriting old data if necessary.
 *
 * This routine writes data to a ring buffer @a buf. If the ring buffer does not
 * have enough space for the new data, it will remove old data to make room by
 * reading and discarding as many records as needed.
 *
 * @warning
 * Use cases involving multiple writers to the ring buffer must prevent
 * concurrent write operations, either by preventing all writers from
