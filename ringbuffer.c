 ...
 */
 ...
 uint32_t ring_buffer_put_overwrite(struct ring_buf *buf, const uint8_t *data, uint32_t size) {
     uint32_t space_needed = 0;
     if (ring_buf_space_get(buf) < size) {
         space_needed = size - ring_buf_space_get(buf);
         uint32_t record_size = size;
         uint32_t records_to_remove = (space_needed + record_size - 1) / record_size;
         uint8_t dummy[record_size];
 
         for (uint32_t i = 0; i < records_to_remove; i++) {
             if (ring_buf_get(buf, dummy, record_size) != record_size) {
                 break;
             }
         }
     }
